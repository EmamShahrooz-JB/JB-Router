---
name: jb-router-web-fetch
description: Fetch URL → markdown / text / HTML via JB-Router /v1/web/fetch using Ollama Cloud / Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---
...
```bash
curl $JB_ROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$JB_ROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $JB_ROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $JB_ROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $JB_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://github.com/EmamShahrooz-JB/JB-Router","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.JB_ROUTER_URL}/v1/web/fetch`, {
  method: "POST",
