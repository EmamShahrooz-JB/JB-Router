...
JB-RouterをOpenAI Codex CLIと統合し、OpenAI APIリクエストをJB-Routerのインテリジェントルーティングシステム経由でルーティングします。
...
1. JB-RouterダッシュボードでAPIキーが正しいか確認
...
1. モデル名がJB-Router設定と一致するか確認
2. JB-RouterダッシュボードでOpenAIプロバイダー接続がアクティブか確認
3. 接続されたプロバイダーでモデルが利用可能か確認

## クラウドエンドポイント

localhostの代わりにJB-Routerクラウドエンドポイントを使用するには:

```bash
export OPENAI_BASE_URL="https://github.com/EmamShahrooz-JB/JB-Router"
```

JB-RouterクラウドダッシュボードでAPIキーが設定されていることを確認してください。
