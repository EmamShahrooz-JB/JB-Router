...
**JB-Routerは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、JB-Router自体は100%無料でオープンソースです。**
...
**JB-Routerはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## JB-Routerをセルフホストできますか?

**はい! JB-Routerは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.jb-router`)
...
**はい、JB-Routerはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.jb-router`(またはカスタム`DATA_DIR`)にローカル保存
- JB-Routerサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](github.com/EmamShahrooz-JB/JB-Router/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.jb-router`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](github.com/EmamShahrooz-JB/JB-Router/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [jb-router.com/docs](https://github.com/EmamShahrooz-JB/JB-Router#documentation)
