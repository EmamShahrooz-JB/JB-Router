...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that JB-Router tries in order.
...
JB-Router automatically tries each model in sequence until one succeeds.
...
Cursor and Claude Code send **unprefixed** model IDs (`composer-2.5`, `claude-opus-5`, `opus`), while JB-Router routes with provider prefixes (`cu/composer-2.5`, `cc/claude-opus-5`). Default combo generators bridge that gap.

On **Dashboard → Combos**:

1. Click **Cursor Default** or **Claude Default**
2. Confirm the preview (new vs already-existing names)
3. JB-Router creates one combo per client model ID, seeded with the matching prefixed route

**Examples:**

| Combo name (what the client sends) | Seeded model (what JB-Router routes) |
|------------------------------------|------------------------------------|
...
> These combos help when Cursor/Claude already talk to JB-Router (`/v1` or `ANTHROPIC_BASE_URL`) and send their native model IDs. They do not change Cursor’s built-in Models tab by themselves.
...
When limit reached, JB-Router skips paid models and uses free tier only.
