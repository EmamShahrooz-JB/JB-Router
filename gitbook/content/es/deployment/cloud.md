...
Despliega JB-Router en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.jb-router` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/jb-router
sudo chown $USER:$USER /var/lib/jb-router
```
...
Crea `/etc/nginx/sites-available/jb-router`:
...
sudo ln -s /etc/nginx/sites-available/jb-router /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de JB-Router
...
# Actualizar JB-Router
cd /path/to/jb-router/app
git pull
npm install
npm run build
pm2 restart jb-router
...
tar -czf jb-router-backup-$(date +%Y%m%d).tar.gz /var/lib/jb-router

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/jb-router-$(date +\%Y\%m\%d).tar.gz /var/lib/jb-router
```
...
```bash
# Verificar si JB-Router está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/jb-router
chmod 755 /var/lib/jb-router
```
