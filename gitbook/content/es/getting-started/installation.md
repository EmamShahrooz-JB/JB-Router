...
Guía detallada de instalación de JB-Router con consejos de solución de problemas.
...
Instala JB-Router globalmente para usar desde cualquier lugar:
...
- ✅ Comando simple: `jb-router`
- ✅ Auto-actualizaciones con `npm update -g jb-router`
...
3. Se crea el directorio de datos en `~/.jb-router`
...
**Ubicación por defecto:** `~/.jb-router`
...
**Error:**
```
Error: EACCES: permission denied, mkdir '/usr/local/lib/node_modules/jb-router'
```
...
**Problema:** JB-Router usa demasiada RAM
...
```bash
npm install -g pm2
pm2 start jb-router --name jb-router
pm2 save
```
...
```bash
docker pull jb-router/jb-router:latest

docker run -d \
  -p 20128:20128 \
  -e JWT_SECRET="your-secure-secret" \
  -e INITIAL_PASSWORD="your-password" \
  -v jb-router-data:/root/.jb-router \
  --name jb-router \
  jb-router/jb-router:latest
```
...
nano ~/.bashrc  # o ~/.zshrc
# Eliminar exports relacionados con jb-router
```
