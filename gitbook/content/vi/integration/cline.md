...
Tích hợp JB-Router với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của JB-Router.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- JB-Router đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint JB-Router:

**Cho JB-Router cục bộ:**
```
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng JB-Router để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua JB-Router
...
3. Nhận giải thích AI qua JB-Router

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng JB-Router để hiểu context và thực hiện thay đổi
...
1. Xác minh JB-Router đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh JB-Router instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs JB-Router để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng JB-Router cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://github.com/EmamShahrooz-JB/JB-Router`
2. Đảm bảo bạn đã cấu hình API key trong JB-Router cloud dashboard
...
## Tích hợp với Tính năng JB-Router

### Định tuyến Model

JB-Router tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, JB-Router tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua JB-Router dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
