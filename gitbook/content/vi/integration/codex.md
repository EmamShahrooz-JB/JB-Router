...
Tích hợp JB-Router với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của JB-Router.
...
JB-Router cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong JB-Router dashboard
...
1. Xác minh tên model khớp với cấu hình JB-Router
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong JB-Router dashboard
...
Đảm bảo bạn đã cấu hình API key trong JB-Router cloud dashboard.
