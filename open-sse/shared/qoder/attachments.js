 ...
 * data-URIs instead, which jb-router previously forwarded verbatim — 10MB
...
export function buildMultipartFile(buffer, { fieldName = "file", fileName, mediaType } = {}) {
  const boundary = `----jb-routerQoder${Date.now().toString(16)}${Math.random().toString(16).slice(2)}`;
