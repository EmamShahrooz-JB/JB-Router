  ...
  return new TransformStream({
    transform(chunk, controller) {
      ...
      for (const line of lines) {
        ...
        if (parsed && parsed.done && targetFormat !== FORMATS.OLLAMA) {
          ...
          // This flag means emitted to the client, not merely consumed upstream.
          // OpenAI chat completion translations emit their sentinel after the
          // translator's final chunks in flush(). Native Gemini/Claude do not.
    ...
    flush(controller) {
      ...
      try {
        ...
        // Gemini/Antigravity ends with a finishReason but has no [DONE] frame.
        // A translated OpenAI chat stream still requires exactly one sentinel,
        // AFTER finish/usage and any translator flush output. Decide using the
        // client format, not the provider name (native Gemini rejects [DONE]).
