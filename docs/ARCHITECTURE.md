# JB-Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

JB-Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[JB-Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.jb-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.jb-router/usage.json`, `~/.jb-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.jb-router/db.json`)
- usage stats: `~/.jb-router/usage.json`
- request log lines: `~/.jb-router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[JB-Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.jb-router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/jb-router && npm run build`
- Build Docker image: `cd /root/dev/jb-router && docker build -t jb-router .`
