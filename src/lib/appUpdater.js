...
// Collect PIDs of all jb-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from jb-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("jb-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("jb-router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "jb-router");
  }
  return path.join(os.homedir(), ".jb-router");
}
...
// Resolve npx/jb-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx jb-router` — works regardless of global bin path changes after npm i -g
