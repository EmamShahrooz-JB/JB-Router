...
export async function runMigrationOnce(adapter) {
  ...
  // No host filesystem backups or legacy file import in Durable Objects.
  // Keep the actual versioned migrations, schema sync and metadata in SQLite.
  if (adapter.driver === "durable-sqlite") {
    adapter.transaction(() => {
