...
const hasJbRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "JB-Router");
};

const getJbRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "JB-Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getJbRouterEntry(config);
...
// POST - Apply JB-Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing JB-Router entry or append
    const idx = config.findIndex((e) => e.name === "JB-Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove JB-Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "JB-Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "JB-Router removed from Copilot config",
    });
  } catch (error) {
