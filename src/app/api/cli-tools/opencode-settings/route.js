...
const hasJbRouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["jb-router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["jb-router"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("jb-router/") ? config.model.replace(/^jb-router\//, "") : null,
...
// POST - Apply JB-Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_jbrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing jb-router provider entry and its models
    const existingProvider = config.provider["jb-router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["jb-router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `jb-router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `jb-router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("jb-router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove JB-Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["jb-router"]?.models) {
      delete config.provider["jb-router"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["jb-router"].models).length === 0) {
        delete config.provider["jb-router"];
        if (config.model?.startsWith("jb-router/")) delete config.model;
      } else if (config.model === `jb-router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["jb-router"].models);
        config.model = `jb-router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire jb-router provider
      if (config.provider) delete config.provider["jb-router"];
      if (config.model?.startsWith("jb-router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("jb-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "JB-Router settings removed from OpenCode",
    });
  } catch (error) {
