...
// Check if settings has JB-Router config
const hasJbRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["jb-router"];
};

// Read per-agent models.json and return current model id (without "jb-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["jb-router"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasJbRouter: hasJbRouterConfig(settings),
...
// POST - Update JB-Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `jb-router/${model}`;

    // Remove all old jb-router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("jb-router/"))
    ...
    // Add fresh jb-router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`jb-router/${m}`] = {};
    });

    // Remove old jb-router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("jb-router/")) {
    ...
    // Update models.providers.jb-router with all models
    settings.models.providers["jb-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `jb-router/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove JB-Router settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove JB-Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["jb-router"];
    ...
    // Remove jb-router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("jb-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses jb-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("jb-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "JB-Router settings removed successfully",
    });
  } catch (error) {
