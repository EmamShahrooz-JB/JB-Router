...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-jb-router.env");
};
...
const hasJbRouterConfig = (config) => {
  ...
  if (providers["jb-router"]) return true;
...
export async function GET() {
  ...
  const hasJbRouter = hasJbRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_JB_ROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile jb-router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["jb-router"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_JB_ROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jb-router configuration removed from jcode",
    });
  } catch (error) {
