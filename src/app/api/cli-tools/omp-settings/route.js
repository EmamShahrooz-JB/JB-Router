...
const hasJbRouterInYml = (content) => {
  if (!content) return false;
  return content.includes("jb-router:") || content.includes("localhost:20128");
};

// Build standard JB-Router provider block for models.yml
const buildOmpProviderYaml = (baseUrl, apiKey) => {
  const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;
  const key = apiKey || "sk_jbrouter";
  return `  ${PROVIDER_ID}:
...
export async function GET() {
  try {
    ...
    const hasJbRouter = hasJbRouterInYml(ymlContent);
...
export async function POST(request) {
  ...
  try {
    ...
    // Remove existing jb-router provider if present
    ...
    try {
      let Database;
      try {
        const mod = await import(String.fromCharCode(98,101,116,116,101,114,45,115,113,108,105,116,101,51));
      ...
      if (Database) {
        ...
        db.prepare(
          ...
          PROVIDER_ID,
          "api_key",
          JSON.stringify({ apiKey: apiKey || "sk_jbrouter", baseUrl }),
    ...
    return NextResponse.json({
      success: true,
      message: "Oh My Pi settings applied! Run 'omp' and all JB-Router models appear under jb-router in /model.",
      configPath: getOmpModelsYmlPath(),
    });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "JB-Router removed from Oh My Pi",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: err.message } }, { status: 500 });
  }
