...
// Check if settings has JB-Router customModels
const hasJbRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:JB-Router"));
};
...
// POST - Update JB-Router customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing JB-Router configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:JB-Router"));
...
// DELETE - Remove JB-Router customModels only (keep other settings)
