...
import { fetchInternalApi } from "@/lib/network/internalApi.js";
...
export async function pingModelByKind(model, kind, baseUrl) {
  ...
  if (kind === "embedding") {
    const res = await fetchInternalApi("/api/v1/embeddings", {
      method: "POST",
      headers,
    ...
    if (!res.ok) {
      return { latencyMs, ...describeUpstreamFailure(res.status, parsed, rawText) };
    }
  ...
  if (kind === "image") {
    const res = await fetchInternalApi("/api/v1/images/generations", {
      method: "POST",
      headers,
  ...
  if (kind === "stt") {
    ...
    const res = await fetchInternalApi("/api/v1/audio/transcriptions", {
      method: "POST",
  ...
  if (kind === "systemone") {
    const res = await fetchInternalApi("/api/v1/systemone", {
      method: "POST",
      headers,
      body: JSON.stringify({
        model,
  ...
  const res = await fetchInternalApi("/api/v1/chat/completions", {
    method: "POST",
    headers,
    body: JSON.stringify({
      model,
