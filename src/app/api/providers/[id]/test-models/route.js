...
import { pingModelByKind, getInternalHeaders } from "@/app/api/models/test/ping";
...
export async function POST(request, { params }) {
  try {
    ...
    if (isCompatible && models.length === 0) {
      try {
        const modelsRes = await fetchInternalApi(`/api/providers/${id}/models`, {
    ...
    const firstResult = await pingModelByKind(`${alias}/${first.id}`, firstKind);
    const results = [{ modelId: first.id, name: first.name || first.id, ...firstResult }];

    if (rest.length > 0) {
      const restResults = await Promise.all(
        rest.map(async (model) => {
          const result = await pingModelByKind(`${alias}/${model.id}`, model.kind || model.type || "llm");
