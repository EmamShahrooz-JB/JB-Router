...
export default function OpenClawToolCard({
  ...
  const currentBaseUrl = openclawStatus?.settings?.models?.providers?.["jb-router"]?.baseUrl || "";

  const getConfigStatus = () => {
    if (!openclawStatus?.installed) return null;
    const currentProvider = openclawStatus.settings?.models?.providers?.["jb-router"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["jb-router"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) setSelectedModel(primaryModel.replace("jb-router/", ""));
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `jb-router/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if jb-router is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["jb-router"]?.baseUrl && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <button onClick={() => { setAgentModalFor(null); setModalOpen(true); }} disabled={!hasActiveProviders} className="jb-select-btn w-full sm:w-auto rounded px-2 py-2 text-xs sm:py-1.5 whitespace-nowrap sm:shrink-0">Select</button>
                </div>
                ...
                {(openclawStatus.agents || []).filter(a => a.agentDir).map((agent) => (
                  <div key={agent.id} className="flex items-center gap-2 pl-4">
                    ...
                    <button onClick={() => { setAgentModalFor(agent.id); setModalOpen(true); }} disabled={!hasActiveProviders} className="jb-select-btn w-full sm:w-auto rounded px-2 py-2 text-xs sm:py-1.5 whitespace-nowrap sm:shrink-0">Select</button>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!openclawStatus?.hasJbRouter} loading={restoring}>
