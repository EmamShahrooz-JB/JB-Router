...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_jbrouter for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# JB-Router Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "jb-router"

[model_providers.jb-router]
name = "JB-Router"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[model_providers.jb-router.http_headers]
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <button onClick={() => setModalOpen(true)} disabled={!activeProviders?.length} className="jb-select-btn w-full sm:w-auto rounded px-2 py-2 text-xs sm:py-1.5 whitespace-nowrap sm:shrink-0">Select Model</button>
                </div>
