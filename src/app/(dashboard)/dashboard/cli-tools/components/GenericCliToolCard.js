...
export default function GenericCliToolCard({
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    ...
    return cfg?.model || cfg?.openai?.model || cfg?.providers?.["jb-router"]?.models?.[0]?.id || "";
  });
  const [selectedModels, setSelectedModels] = useState(() => {
    const cfg = initialStatus?.config;
    const list = cfg?.providers?.["jb-router"]?.models;
        ...
        .then((data) => {
          if (active) {
            ...
            if (tool.id === "pi") {
              ...
              const mod = cfg?.model || cfg?.openai?.model || cfg?.providers?.["jb-router"]?.models?.[0]?.id;
  ...
  const getCurrentBaseUrl = () => {
    ...
    if (typeof cfg.providers?.["jb-router"]?.base_url === "string") return cfg.providers["jb-router"].base_url;
    if (typeof cfg.providers?.["jb-router"]?.baseUrl === "string") return cfg.providers["jb-router"].baseUrl;
    return "";
  };
  ...
  const getConfigStatus = () => {
    ...
    if (!status.hasJbRouter) return "not_configured";
  ...
  const handleApply = async () => {
    ...
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_jbrouter" : selectedApiKey);
  ...
  const getManualConfigContent = () => {
    ...
    const key = selectedApiKey || "sk_jbrouter";
        ...
        return [
          {
            ...
            content: `# Forge config — managed by JB-Router\n\n[openai]\napi_key = "${key}"\nbase_url = "${effectiveUrl}"\nmodel = "${mod}"`,
          },
        ];
      case "smelt":
        return [
          {
            ...
            content: JSON.stringify({ baseUrl: effectiveUrl, apiKey: key, model: mod, _managedBy: "jb-router" }, null, 2),
          },
        ];
      case "codewhale":
        return [
          {
            ...
            content: `# CodeWhale config — managed by JB-Router\n\n[openai]\nbase_url = "${effectiveUrl}"\napi_key = "${key}"\nmodel = "${mod}"`,
          },
        ];
      default:
        return [];
                ...
                {tool.id !== "omp" && tool.id !== "pi" && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <button
                      onClick={() => setModalOpen(true)}
                      disabled={!activeProviders?.length}
                      className="jb-select-btn w-full sm:w-auto rounded px-2 py-2 text-xs sm:py-1.5 whitespace-nowrap sm:shrink-0"
                    >
                      Select Model
                    </button>
                  </div>
