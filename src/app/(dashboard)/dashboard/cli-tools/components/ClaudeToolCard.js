...
export default function ClaudeToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      // Get key from dropdown, fallback to first key or sk_jbrouter for localhost
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <button onClick={() => openModelSelector(model.alias)} disabled={!hasActiveProviders} className="jb-select-btn w-full sm:w-auto rounded px-2 py-2 text-xs sm:py-1.5 whitespace-nowrap sm:shrink-0">Select Model</button>
                  </div>
                ))}
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                  <select value={autoCompactWindow} onChange={(e) => setAutoCompactWindow(e.target.value)} className="jb-select jb-select--sm w-full min-w-0">
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasJbRouter} loading={restoring}>
