...
export async function getProviderCredentials(provider, excludeConnectionIds = null, model = null, options = {}) {
  ...
  // Explicit dashboard probes bypass recorded cooldowns so the user sees the
  // real upstream answer instead of a cached "all accounts locked" 503.
  ...
  try {
    ...
    const availableConnections = connections.filter(c => {
      ...
      if (!ignoreModelLocks && isAntigravity && model && antigravityQuotaCache) {
    ...
    log.debug("AUTH", `${provider} | available: ${availableConnections.length}/${connections.length}${ignoreModelLocks ? " (force-test: cooldowns ignored)" : ""}`);
    ...
    if (availableConnections.length === 0) {
      ...
      // Explicit probes ignore cooldowns entirely, so they report "no credentials"
      // instead of replaying a lock the probe is not subject to.
      const lockedConns = ignoreModelLocks ? [] : connections.filter(c => isModelLockActive(c, model));
