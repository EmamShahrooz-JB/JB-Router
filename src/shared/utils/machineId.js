...
const CLI_AUTH_SALT = 'jb-router-cli-auth';
...
function loadRawMachineId() {
  ...
  if (typeof navigator !== 'undefined' && navigator.userAgent === 'Cloudflare-Workers') {
    if (!process.env.JWT_SECRET) throw new Error("JWT_SECRET must be configured on Workers");
    cachedRawId = crypto.createHmac('sha256', process.env.JWT_SECRET).update('jb-router-machine-id-v1').digest('hex');
    return cachedRawId;
  }
...
function loadCliSecret() {
  ...
  if (typeof navigator !== 'undefined' && navigator.userAgent === 'Cloudflare-Workers') {
    if (!process.env.JWT_SECRET) throw new Error("JWT_SECRET must be configured on Workers");
    cachedCliSecret = crypto.createHmac('sha256', process.env.JWT_SECRET).update('jb-router-cli-secret-v1').digest('hex');
