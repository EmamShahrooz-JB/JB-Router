...
const cliAppDir = process.env.JB_ROUTER_CLI_APP_DIR || path.join(cliDir, "app");
...
function resolveStandaloneBuild(appDir, buildDistDir) {
  ...
  // NEXT_TRACING_ROOT_MODE=workspace, e.g. standalone/jb-router/server.js.
...
function buildCliPackage() {
  console.log("📦 Building JB-Router CLI package with Next.js...\n");
  ...
  // Strip better-sqlite3 (native) — it lives in ~/.jb-router/runtime to avoid
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (lives in ~/.jb-router/runtime)");
  }
  console.log("");
