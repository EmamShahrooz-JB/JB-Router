...
async function buildCodexHeader() {
  ...
  const { installed, hasJbRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasJbRouter, settings } = result.data;
  ...
  // Extract JB-Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:JB-Router-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract JB-Router provider config
  const provider = settings?.models?.providers?.["jb-router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("jb-router/") ? primary.replace("jb-router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasJbRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use JB-Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
