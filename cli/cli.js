...
// Subcommands (`jb-router xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.jb-router/runtime
...
// Identifiers for killAllAppProcesses - only kill jb-router specifically
const PROCESS_IDENTIFIERS = [
  'jb-router'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "jb-router")
    : path.join(os.homedir(), ".jb-router");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running jb-router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "jb-router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("jb-router") && (cmd.includes("cli.js") || cmd.includes("\\jb-router") || cmd.includes("/jb-router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running jb-router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "jb-router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("jb-router") && (cmd.includes("cli.js") || cmd.includes("/jb-router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 JB-Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 JB-Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "jb-router", "db.json") : path.join(".jb-router", "db.json"));
