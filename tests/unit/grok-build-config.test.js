...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("jb-router");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "jb-router-general-purpose",
      explore: "jb-router-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.jb-router\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.jb-router-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.jb-router-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# jb-router-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.jb-router-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("jb-router-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.jb-router-");
    expect(reset).not.toContain("jb-router-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("jb-router-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("jb-router-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("jb-router-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("jb-router-explore");
    expect(getGrokSubagentSlot("plan")).toBe("jb-router-plan");
