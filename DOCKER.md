...
Run JB-Router in a container. Published image: [`EmamShahrooz-JB/JB-Router`](https://github.com/EmamShahrooz-JB/JB-Router) — multi-platform `linux/amd64` + `linux/arm64`.
...
```bash
docker logs -f jb-router        # view logs
docker stop jb-router           # stop
docker start jb-router          # start again
docker rm -f jb-router          # remove
```
...
Without `DATA_DIR`, the app falls back to `~/.jb-router/` (macOS/Linux) or `%APPDATA%\jb-router\` (Windows). In the container, `DATA_DIR=/app/data` makes the bind mount work.
...
Host path: `$HOME/.jb-router/db/data.sqlite`
...
The JB-Router image does not bundle Python or Headroom. To use Headroom in Docker, run it as a separate service and point JB-Router at that proxy:

```yaml
services:
  jb-router:
    image: EmamShahrooz-JB/JB-Router:latest
    ports:
      - "20128:20128"
    volumes:
      - "$HOME/.jb-router:/app/data"
    environment:
      DATA_DIR: /app/data
...
```bash
docker pull EmamShahrooz-JB/JB-Router:latest
docker rm -f jb-router
...
```bash
docker pull EmamShahrooz-JB/JB-Router:0.5.81
```
...
- `ghcr.io/EmamShahrooz-JB/JB-Router:X.Y.Z` + `:latest`
- `EmamShahrooz-JB/JB-Router:X.Y.Z` + `:latest`
...
```bash
docker pull EmamShahrooz-JB/JB-Router@sha256:<verified-digest>
```
...
GHCR publishing uses the workflow's `GITHUB_TOKEN` with package write permission. Forks can publish to their own GHCR namespace, but Docker Hub publication is restricted to the upstream `EmamShahrooz-JB/JB-Router` repository.
