<div align="center">
  <img src="../images/jb-router.png?2" alt="Tableau de bord JB-Router" width="800"/>
  
  # JB-Router - Routeur IA GRATUIT et économiseur de tokens
...
[🚀 Démarrage rapide](#-démarrage-rapide) • [💡 Fonctionnalités](#-fonctionnalités-clés) • [📖 Configuration](#-guide-dinstallation) • [🌐 Site web](https://github.com/EmamShahrooz-JB/JB-Router)
...
Le paquet de ce dépôt est privé (`jb-router-app`), donc l'exécution depuis le code source/Docker est le chemin de développement local attendu.
...
> 🎬 **Vous avez réalisé une vidéo sur JB-Router ?** Envoyez une [Pull Request](github.com/EmamShahrooz-JB/JB-Router/pulls) en ajoutant votre vidéo à cette section — nous la fusionnerons !

---

## 🛠️ Outils CLI pris en charge

JB-Router fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. JB-Router appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → JB-Router → Headroom /v1/compress → JB-Router → fournisseur
```
...
Si Headroom est indisponible ou renvoie une erreur, JB-Router bascule en mode dégradé et envoie la requête originale.
...
- Votre outil CLI envoie le format OpenAI → JB-Router le traduit → Le fournisseur reçoit le format natif
...
> JB-Router lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de JB-Router

**La réalité de la facturation de JB-Router :**

✅ **Le logiciel JB-Router = GRATUIT pour toujours** (open source, ne facture jamais)  
...
❌ **JB-Router n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, JB-Router se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **JB-Router** : Ne facture jamais rien, jamais
...
<details>
<summary><b>💳 JB-Router me facturera-t-il ?</b></summary>

**Non.** JB-Router est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, JB-Router route simplement vos requêtes
- ❌ **JB-Router lui-même** → **Ne facture jamais rien, jamais**

JB-Router est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
JB-Router se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et JB-Router les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - JB-Router vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de JB-Router évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et JB-Router les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. JB-Router suit le quota par modèle !
...
Settings → Models → Advanced :
  ...
  OpenAI API Key : [depuis le tableau de bord jb-router]
...
```json
{
  ...
  "anthropic_api_key": "votre-clé-api-jb-router"
}
```
...
export OPENAI_API_KEY="votre-clé-api-jb-router"
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un JB-Router local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
docker pull EmamShahrooz-JB/JB-Router:latest   # mise à jour vers la dernière version
```

**Persistance des données :** `$HOME/.jb-router/db/data.sqlite` sur l'hôte ↔ `/app/data/db/data.sqlite` dans le conteneur.

### Variables d'environnement

| Variable                                             | Par défaut                                  | Description                                                                                  |
| ---------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Généré automatiquement (`~/.jb-router/jwt-secret`) | Secret de signature JWT pour le cookie d'authentification du tableau de bord (remplacez-le pour partager entre instances) |
| `INITIAL_PASSWORD`                                   | `123456`                                    | Mot de passe de première connexion quand aucun hash n'est enregistré                          |
| `DATA_DIR`                                           | `~/.jb-router`                                | Emplacement principal des données de l'app (SQLite dans `$DATA_DIR/db/data.sqlite`)          |
...
| `CLOUD_URL`                                          | `https://github.com/EmamShahrooz-JB/JB-Router`                       | URL de base de l'endpoint de synchronisation cloud côté serveur                               |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                     | URL de base publique/rétrocompatible (préférez `BASE_URL` pour le runtime serveur)            |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://github.com/EmamShahrooz-JB/JB-Router`                       | URL cloud publique/rétrocompatible (préférez `CLOUD_URL` pour le runtime serveur)             |
...
- `${DATA_DIR}` et `~/.jb-router` résolvent tous deux le même emplacement dans un conteneur Docker — le lien symbolique `/root/.jb-router -> /app/data` est créé au moment de la compilation.
...
- Renouvelé automatiquement par JB-Router
...
- **Site web** : [jb-router.com](https://github.com/EmamShahrooz-JB/JB-Router)
- **GitHub** : [github.com/EmamShahrooz-JB/JB-Router](github.com/EmamShahrooz-JB/JB-Router)
- **Problèmes** : [github.com/EmamShahrooz-JB/JB-Router/issues](github.com/EmamShahrooz-JB/JB-Router/issues)

---

## 👥 Contributeurs

Merci à tous les contributeurs qui ont contribué à améliorer JB-Router !
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork TypeScript complet de JB-Router. Ajoute plus de 36 fournisseurs, un repli automatique à 4 niveaux, des API multimodales (images, embeddings, audio, TTS), un disjoncteur, un cache sémantique, des évaluations de LLM et un tableau de bord soigné. Plus de 368 tests unitaires. Disponible via npm et Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. JB-Router porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. JB-Router adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. JB-Router injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de JB-Router n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
