<div align="center">
  ...
  **Hubungkan semua tool AI coding (Claude Code, Cursor, Antigravity, Copilot, Codex, Gemini, OpenCode, Cline, OpenClaw...) ke 40+ provider AI dan 100+ model.**
  ...
  [🚀 Mulai Cepat](#-mulai-cepat) • [💡 Fitur](#-fitur-utama) • [📖 Setup](#-panduan-setup) • [🌐 Website](https://github.com/EmamShahrooz-JB/JB-Router)
...
**JB-Router menyelesaikan itu semua:**
...
Paket repo ini bersifat privat (`jb-router-app`), jadi menjalankan dari source/Docker adalah jalur yang diharapkan untuk pengembangan lokal.
...
### 📺 Panduan Setup Lengkap - JB-Router + Claude Code Gratis
...
JB-Router bekerja mulus dengan semua tool AI coding utama:
...
- Tool CLI mengirim dalam format OpenAI → JB-Router mengonversi → provider menerima dalam format nativenya
...
> JB-Router sendiri **tidak menagih apa pun**. Kamu hanya membayar langsung ke provider jika memakai layanan berbayar.
>
...
>
> Anggap saja ini "pelacak penghematan" yang menunjukkan berapa banyak yang kamu hemat lewat model gratis dan routing JB-Router!
...
### 📊 Tentang Biaya dan Penagihan JB-Router

**Fakta soal penagihan JB-Router:**

✅ **Software JB-Router = gratis selamanya** (open source, tanpa tagihan)
...
❌ **JB-Router tidak mengirim invoice** atau menagih kartumu
...
- **Provider murah** (GLM, MiniMax): bayar langsung, JB-Router hanya melakukan routing
- **Provider gratis** (iFlow, Kiro, Qwen): benar-benar gratis selamanya, tanpa biaya tersembunyi
- **JB-Router**: tidak menagih apa pun
...
Dashboard melacak pemakaian token dan menampilkan **estimasi biaya** seandainya kamu memakai API berbayar secara langsung. Ini **bukan tagihan nyata**, melainkan acuan untuk melihat berapa banyak yang kamu hemat dengan memakai model gratis atau langganan yang sudah ada lewat JB-Router.
...
<details>
<summary><b>💳 Apakah JB-Router menagih saya?</b></summary>

**Tidak.** JB-Router adalah software open source gratis yang berjalan di komputermu sendiri. Tidak ada penagihan sama sekali.

**Kamu membayar ke:**
- ✅ **Provider langganan** (Claude Code $20/bulan, Codex $20-200/bulan) → bayar langsung di website masing-masing
- ✅ **Provider murah** (GLM, MiniMax) → bayar langsung, JB-Router hanya me-routing request
- ❌ **JB-Router sendiri** → **tidak menagih apa pun**

JB-Router adalah proxy/router lokal. Ia tidak menyimpan informasi kartu kredit, tidak bisa mengirim invoice, dan tidak punya sistem penagihan. Sepenuhnya software gratis.
...
JB-Router hanya me-routing request — tidak ada "jebakan" atau tagihan di kemudian hari. Layanannya memang gratis, dan JB-Router membuatnya lebih mudah dipakai dengan dukungan fallback.

**Catatan:** beberapa provider langganan (Antigravity, GitHub Copilot) punya masa preview gratis dan bisa jadi berbayar nanti, tetapi hal itu diumumkan secara jelas oleh provider tersebut, bukan oleh JB-Router.
...
3. **Gunakan provider langganan paling akhir:**
   ...
   - JB-Router memaksimalkan nilainya lewat pelacakan kuota
...
Smart fallback JB-Router mencegah tagihan tak terduga:
...
**Kamu yang pegang kendali:** atur batas pengeluaran per provider di dashboard, dan JB-Router akan mematuhinya.
...
**Tips pro:** pakai Opus untuk tugas kompleks, Sonnet kalau mengutamakan kecepatan. JB-Router melacak kuota per model!
...
docker run -d \
  ...
  ghcr.io/EmamShahrooz-JB/JB-Router:latest
```
...
```bash
npm install -g jb-router-cli
PORT=20128 HOSTNAME=0.0.0.0 BASE_URL=https://your-domain.com jb-router
```
...
MIT License — lihat [LICENSE](github.com/EmamShahrooz-JB/JB-Router/blob/main/LICENSE) untuk detailnya.

---

<div align="center">

**Kalau JB-Router membantumu, kasih ⭐ di [GitHub](github.com/EmamShahrooz-JB/JB-Router)!**

[🌐 Website](https://github.com/EmamShahrooz-JB/JB-Router) • [📦 npm](https://github.com/EmamShahrooz-JB/JB-Router#cli) • [🐛 Laporkan Bug](github.com/EmamShahrooz-JB/JB-Router/issues)
