...
const tracingRoot = process.env.NEXT_TRACING_ROOT_MODE === "workspace"
  ? join(projectRoot, "..")
  : projectRoot;
const proxyClientMaxBodySize = process.env.JB_ROUTER_PROXY_CLIENT_MAX_BODY_SIZE || "128mb";
...
const nextConfig = {
  ...
  env: {},
  experimental: {
    // Keep full builds within the deployment sandbox's memory budget.
    cpus: 1,
  ...
  webpack: (config, { isServer, nextRuntime }) => {
    // Edge runtime (middleware/proxy) cannot use native sqlite — mark as external
    if (isServer && nextRuntime === "edge") {
      config.externals = [...(config.externals || []), "bun:sqlite", "better-sqlite3", "node:sqlite"];
    }
